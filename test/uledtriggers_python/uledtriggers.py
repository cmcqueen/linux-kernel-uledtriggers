...
class IOCTL(EnumMissingUnknown, IntEnum):
    ...
    ULEDTRIGGERS_IOC_BLINK_ONESHOT  = 0x40145413
...
def uledtriggers_blink(uledtriggers_f):
    for i, (_name, f) in enumerate(uledtriggers_f):
        ...
        blink_struct = struct.pack('QQI', 1000, 2000, invert)
        fcntl.ioctl(f, IOCTL.ULEDTRIGGERS_IOC_BLINK, blink_struct)
    while True:
        time.sleep(1)

def uledtriggers_periodic_write(uledtriggers_f):
    while True:
        time.sleep(2)
        for i, (_name, f) in enumerate(uledtriggers_f):
...
def main():
    ...
    try:
        #uledtriggers_periodic_write(uledtriggers_f)
